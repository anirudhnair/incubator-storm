...
public class ThroughputVsLatency {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (args != null && args.length > 4) {
            batch_interval = Integer.valueOf(args[4]);
        }
        ...
        conf.put(Config.TOPOLOGY_DISRUPTOR_BATCH_SIZE, batch_size);
        conf.put(Config.TOPOLOGY_DISRUPTOR_BATCH_TIMEOUT_MILLIS, batch_interval);
        conf.put(Config.TOPOLOGY_BACKPRESSURE_ENABLE,false);
        TopologyBuilder builder = new TopologyBuilder();

        int numEach = 2 * parallelism;
